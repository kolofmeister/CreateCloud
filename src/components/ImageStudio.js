        ...
        if (type === 'model') {
            ...
            const renderModels = (filter = '') => {
                ...
                filtered.forEach(m => {
                    ...
                    item.innerHTML = `
                        <div class="flex items-center gap-3.5">
                             <div class="w-10 h-10 ${m.family === 'kontext' ? 'bg-blue-500/10 text-blue-400' : m.family === 'effects' ? 'bg-purple-500/10 text-purple-400' : 'bg-primary/10 text-primary'} border border-white/5 rounded-xl flex items-center justify-center font-black text-sm shadow-inner uppercase ${available ? '' : 'opacity-40'}">${m.name.charAt(0)}</div>
                             <div class="flex flex-col gap-0.5">
                                <span class="text-xs font-bold tracking-tight ${available ? 'text-white' : 'text-white/40'}">${m.name}</span>
                             </div>
                        </div>
                        <div class="flex items-center gap-2">
                            ${!available ? '<div class="relative group/unavail w-5 h-5 rounded-full bg-red-500/20 border border-red-500/50 flex items-center justify-center text-red-400 text-[11px] font-black select-none cursor-default" title="Zurzeit nicht bei fal.ai verfügbar.">!</div>' : ''}
